...
public class MyFollow_AbilitySlice extends AbilitySlice implements Component.ClickedListener {
    Component back;
    String userName;
    ...
    List<Entity_Post> getData() {
        return DataUtil.query_my_FollowPost(userName);
    }
    ...
    void setListContainer(AbilitySlice slice) {
        //找到ListContainer
        listContainer = (ListContainer) findComponentById(ResourceTable.Id_myFollow_ListContanier);
        Component notice = findComponentById(ResourceTable.Id_myFollow_notice);
    ...
    public void onStart(Intent intent) {
        ...
        super.setUIContent(ResourceTable.Layout_c_my_follow);
        userName = intent.getStringParam("userName");
        setListContainer(this);
        back = findComponentById(ResourceTable.Id_m2_back);
